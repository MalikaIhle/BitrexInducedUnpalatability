...
contingency_tbl <-   data.frame(AllAttack1 %>%
                                  group_by(AttackedTermitePalatability,PriorExposureYN, DropYN) %>%
                                  summarize( Count = n()))

contingency_tbl$PalatExp <- paste(contingency_tbl$AttackedTermitePalatability,contingency_tbl$PriorExposureYN, sep="-")
...
contingency_tbl2 <-   data.frame(AllAttack1 %>%
...
plot2 <- ggplot(contingency_tbl2, aes(x = factor(AttackedTermitePalatability), y = count_dropped_perc, fill = factor(PriorExposureYN) )) +  
